...
```{r}
#| label: Load Data
# Loads relevant data from library
data(penguins)
```
...
> Insert Answer Here: Variable are species, island, bill length (mm), bill depth(mm), flipper length(mm), body mass(g), sex, and year. 
...
```{r}
#| label: Species Barchart
# Creates barchart of species in the penguins dataset
...
```{r}
#| label: Bill Ratio Scatterplot
# Creates scatterplot of penguins' bill length(mm) vs bill depth(mm)
ggplot(data = penguins) +
...
# Creates scatterplot of penguins' bill length(mm) vs bill depth(mm), with color mapped to species
ggplot(data = penguins) +
  geom_point(mapping = aes(
    x = bill_length_mm, 
    y = bill_depth_mm,
...
# Creates scatterplot of penguins' bill length(mm) vs bill depth(mm), with color mapped to island and shape mapped to species
ggplot(data = penguins) +
  geom_point(mapping = aes(
    x = bill_length_mm, 
    y = bill_depth_mm,
